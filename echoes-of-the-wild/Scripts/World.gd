...
func _process(delta):
	...
	if not map_open:
		...
		player.position.x = clamp(player.position.x, 60.0, WORLD_SIZE.x - 60.0)
		player.position.y = clamp(player.position.y, 80.0, WORLD_SIZE.y - 60.0)
	else:
...
func _draw():
	...
	draw_circle(player.position, 20, Color("#62b5ff"))
	draw_circle(player.position, 7, Color.WHITE)
	if weather == "Rain":
		for p in rain:
